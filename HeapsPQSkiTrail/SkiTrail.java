...
public class SkiTrail
{
    ...
    {
        double ran = (rand.nextDouble()) + 1;
        double down = length/(sk.skillLevel() * ran);
        sk.setTime( sk.time() + down );
        skierPQ.add( sk );
    }
    ...
    {
//        TreeSet<Skier> tree = new TreeSet<Skier>();
//
//        while ( !skierPQ.isEmpty() && skierPQ.peek().time() < clockTime )
//        {
//            tree.add( skierPQ.remove() );
//
//        }
//        return tree;
        TreeSet<Skier> treee = new TreeSet<Skier>();
        Skier sk = skierPQ.peek();
        
        while(sk != null && sk.time() < clockTime)
        {
            treee.add( skierPQ.remove());
            sk = skierPQ.peek();
        }
        return treee; 
    }
