...
public class JMCh24Exercises
{
    ...
    public boolean has3OnSameDay( Date[] birthdays )
    {
        int numBday[] = new int[400];
        
        for (int i = 0; i < birthdays.length; i++) 
        {
            int date = birthdays[i].getMonth() * 31;
            int day = birthdays[i].getDay();
            numBday[date+day]++;
            if (numBday[date+day] == 3) 
            {
                return true;
            }
        }
    ...
    public String mostFrequentWord( Scanner input )
    {
        ...
        {        
            ...
            {
                int val = counts.get( words[i] );
                counts.remove( words[i] );
                counts.put( words[i], val++ );
            }
