...
public class EraseObject extends World<Tile>
{
    ...
    private void getData( String fName )
    {
        ...
        while ( in.hasNextInt() )
        {
            Location loc = new Location( in.nextInt(), in.nextInt() );
            Tile til = new Tile( IMGOBJ_COLOR );
    ...
    private void erase( Grid<Tile> gr, int row, int col )
    {
        ...
        Location tile1 = new Location( row, col );
        
        if ( getGrid().get( tile1 ) == null ) {
            return;
        }
        
        getGrid().get( tile1 ).setColor( ERASE_COLOR );
        for ( int i = row - 1; i <= row + 1; i++ )
        {
            for ( int j = col - 1; j <= col + 1; j++ )
            {
                ...
                if ( getGrid().getOccupiedLocations().contains( loc )
                        && !getGrid().get( loc ).getColor().equals( ERASE_COLOR ) )
                {


                    Location bleh = new Location( loc.getRow(), loc.getCol() );
                    getGrid().get( bleh ).setColor( ERASE_COLOR );

                    erase( getGrid(), loc.getRow(), loc.getCol() );
                }
            }
        }
