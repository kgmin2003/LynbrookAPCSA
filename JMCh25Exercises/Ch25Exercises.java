...
public class Ch25Exercises
{
    ...
    {
        int n = (Integer)completeTree[0];
        int len = ( n + 1 ) / 2;
        for ( int i = n; i >= len; i-- )
        {
            completeTree[i] = null;
        }
        completeTree[0] = n - len;
    }
    ...
    public Object[] toArray( TreeNode root, int numNodes )
    {
        Object[] treee = new Object[numNodes + 1];

        treee[0] = numNodes;
        toArray( root, treee, 1 );
    ...
    private void toArray( TreeNode node, Object[] values, int i )
    {
        ...
        else
        {
            ...
            toArray( node.getLeft(), values, i );
            i += 1;
            toArray( node.getRight(), values, i );
        }
    }
    ...
    public boolean isHeap( Integer[] items, int numNodes )
    {
        ...
        {
            // System.out.println( items[i] + "<" + items[i/2] );
    ...
    public void reheapDown( Integer[] items, int numNodes )
    {
        ...
        {
            if ( items[i] > items[2 * i] || items[i] > items[2 * i + 1] )
            {
                int swap;
