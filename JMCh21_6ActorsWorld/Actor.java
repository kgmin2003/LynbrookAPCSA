...
public class Actor implements Comparable<Actor>
{
    ...
    /**
     * 
     * checks this actor’s possessions and sends requests to
     * everyone for each missing item. After that, readMail “reads” 
     * all messages in this actor’s mailbox and sends replies 
     * or takes appropriate actions
     */
    public void readMail()
    {
        ...
        while (!mailbox.isEmpty())
        {
            ...
            {
                if ( this.countPossessions( item ) > 1 )
                {
                    reply(m.getSender(), "have " + item);
                }
            }
            
            else if (msg.contains( "have" )){
                if (!myPossessions.contains( item ))
                {
                    reply(m.getSender(), "ship " + item);
                }
            }
            
            else if (msg.contains( "ship" ))
            {
                if (countPossessions(item) > 1 )
                {
                    if (m.getSender().receiveItem( this, item ) == true);
                    {
